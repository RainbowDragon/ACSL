...
public class RingsSenior {
     ...
     */
    static String scoreTosses(int numPlayers, List<String> tosses) {

        int[][] scores = new int[numPlayers][3];
        for (int i = 0; i < numPlayers; i++)
        {
            scores[i][0] = i + 1;
            String[] tossList = tosses.get(i).split(" ");
            scores[i][1] = tossList.length;
            scores[i][2] = getScore(tossList);
        }

        Arrays.sort(scores, (x, y) -> x[2] == y[2] ? x[1] - y[1] : y[2] - x[2]);
        ...
        for (int i = 0; i < numPlayers; i++)
        {
            ...
            sb.append(scores[i][0]);
            sb.append("-");
            sb.append(scores[i][2]);
        }
    ...
    static int getScore (String[] tossList) {
    ...
    public static void main (String [] args) {
        ...
        for (int i = 0; i < 12; i++)
        {
            ...
            String result = scoreTosses(Integer.parseInt(input[i][0]), arrays);
