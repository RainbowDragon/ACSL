...
#
def sum_of_min_along_path(dim, arrays):

    rows, cols = (int(x) for x in dim.split())
    matrix = parse_array(arrays, rows, cols)
    ...
    while not visited[pos[0]][pos[1]]:
        ...
        result += find_smallest(matrix, pos[0], pos[1])
        value_dict = {}
        for dr in [-1, 0, 1]:
            if 0 <= pos[0] + dr < rows:
                for dc in [-1, 0, 1]:
