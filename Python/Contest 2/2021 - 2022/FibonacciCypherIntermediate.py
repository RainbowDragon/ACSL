...
# 4. STRING msg
#
def fib_cypher(num1, num2, key, msg):
    ...
    result = ""
    for k in range(n):
        offset = (fib_numbers[k % 20] + ord(key) - ord('a')) % 26 + ord('a')
...
for i in range(10):
    test_result = fib_cypher(test_input[i][0], test_input[i][1], test_input[i][2][0], test_input[i][3])
