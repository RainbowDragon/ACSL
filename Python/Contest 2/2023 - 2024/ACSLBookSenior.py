...
def encode_message(text, message):
    ...
    for ns in range(len(word_list)):
        for nw in range(len(word_list[ns])):
            for nc in range(len(word_list[ns][nw])):
                index = ord(word_list[ns][nw][nc])
                key = str(ns+1) + "." + str(nw+1) + "." + str(nc+1)
                key_list[index].append(key)
    ...
    for j in range(len(message)):
        ...
        if c.isalnum():
            ...
            encode_count += 1
            index = encode_count
            while index > len(key_list[ord(c)]):
                index //= 2

            sb.append(key_list[ord(c)][index-1])
            need_space = True
        elif c == ' ':
            sb.append("_")
            need_space = False
...
for i in range(10):
    test_result = encode_message(test_input[i][0], test_input[i][1])
