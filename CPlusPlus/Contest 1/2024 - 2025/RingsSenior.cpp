...
int getScore(vector<string>& tossList)
{
    int score = 0;

    for (string toss : tossList)
    {
...
{
    vector<vector<int>> scores(numPlayers, vector<int>(3));

    for (int i = 0; i < numPlayers; i++)
    {
        scores[i][0] = i + 1;

        vector<string> tossList;
        stringstream ss(tosses[i]);
        string toss;
        while (getline(ss, toss, ' '))
        {
            tossList.push_back(toss);
        }

        scores[i][1] = tossList.size();
    ...
    for (int i = 0; i < numPlayers; i++)
    {
        ...
        result += to_string(scores[i][0]) + "-" + to_string(scores[i][2]);
    }
...
int main()
{
    ...
    for (int i = 0; i < 12; i++)
    {
        ...
        string result = scoreTosses(stoi(input[i][0]), arrays);
