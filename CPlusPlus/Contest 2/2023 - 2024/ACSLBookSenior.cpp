 ...
 * 2. STRING message
 */
string encodeMessage(string text, string message)
{
    string txt = "";
    ...
    vector<string> keyList[128];
    for (int ns = 0; ns < n; ns++)
        for (int nw = 0; nw < wordlist[ns].size(); nw++)
            for (int nc = 0; nc < wordlist[ns][nw].length(); nc++)
            {
                int index = wordlist[ns][nw][nc];
                string key = to_string(ns+1) + "." + to_string(nw+1) + "." + to_string(nc+1);
                keyList[index].push_back(key);
            }    
